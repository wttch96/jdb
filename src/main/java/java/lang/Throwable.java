...
public class Throwable implements Serializable {
     ...
     * 和相关方法打印的堆栈跟踪元素.
     * <p>
     * 此方法旨在供 RPC 框架和其他高级系统使用, 允许客户端覆盖默认堆栈跟踪,
     * 该跟踪由 {@link #fillInStackTrace()} 在构造 Throwable 时生成或
     * 反序列化时读取 Throwable 来自序列化流.
     *
     * <p>如果此 {@code Throwable} {@linkplain Throwable#Throwable(String, Throwable, boolean, boolean) 堆栈跟踪不可写},
     * 则调用此方法除了验证其参数之外没有任何效果.
     *
     * @param stackTrace 与此 {@code Throwable} 关联的堆栈跟踪元素. 此调用复制指定的数组;
     *                   方法调用返回后指定数组中的更改将不会影响此 {@code Throwable} 的堆栈跟踪.
     * @throws NullPointerException 如果 {@code stackTrace} 为空 {@code null} 或者
     *                              {@code stackTrace} 中存在 {@code null}.
     * @since 1.4
     */
    ...
    /**
     * 从流中读取 {@code Throwable}, 对字段强制执行格式良好的约束. {@code suppressExceptions}
     * 列表中不允许空条目和自指针. 堆栈跟踪元素不允许空条目. 串行形式的空堆栈跟踪导致零长度堆栈元素数组.
     * 单元素堆栈跟踪导致的 {@code null} {@code stackTrace} 字段,
     * 其条目等于 {@code new StackTraceElement("", "", null, Integer.MIN_VALUE)} .
     * <p>
     * 请注意, {@code cause} 字段可以容纳的值没有限制; {@code null} 和 {@code this} 都是该字段的有效值.
     */
    private void readObject(ObjectInputStream s)
        ...
        if (suppressedExceptions != null) {
            ...
            } else { // 将 Throwables 复制到新列表
                suppressed = new ArrayList<>(1);
                for (Throwable t : suppressedExceptions) {
                    // 在损坏或恶意流的情况下对受抑制的异常实施约束.
                    if (t == null)
        ...
        } // 否则 null 的 suppressExceptions 字段保持为 null

        /*
         * 对于零长度堆栈跟踪, 请使用 UNASSIGNED_STACK 的克隆而不是 UNASSIGNED_STACK 本身,
         * 以允许在 getOurStackTrace 中对 UNASSIGNED_STACK 进行身份比较.
         * stackTrace 中 UNASSIGNED_STACK 的标识向 getOurStackTrace 方法指示
         * 需要根据 backtrace 中的信息构造 stackTrace.
         */
        if (stackTrace != null) {
            ...
            // 串行表单中的 null stackTrace 字段可能是由旧版 JDK 中没有该字段的序列化异常引起的;
            // 将此类异常视为具有空堆栈跟踪.
            stackTrace = UNASSIGNED_STACK.clone();
        }
    }
     ...
     * <p>
     * 空堆栈跟踪字段在串行形式中表示为单元素数组, 其元素等于
     * {@code new StackTraceElement("", "", null, Integer.MIN_VALUE)}.
     */
    private synchronized void writeObject(ObjectOutputStream s)
            throws IOException {
        // 如果适用, 请确保将 stackTrace 字段初始化为非 null 值.
        // 从JDK 7开始, 空堆栈跟踪字段是一个有效值, 表示不应设置堆栈跟踪.
        getOurStackTrace();
    ...
    /**
     * 将指定的异常附加到为了传递此异常而被抑制的异常. 此方法是线程安全的,
     * 通常由 {@code try}-with-resources 语句调用(自动和隐式).
     *
     * <p> <em>除非</em> {@linkplain #Throwable(String, Throwable, boolean, boolean) 通过构造函数} 禁用,
     * 否则启动抑制行为. 禁用抑制时, 此方法可以除了验证参数, 别无他用.
     *
     * <p>请注意, 当一个异常 {@linkplain #initCause（Throwable）导致}另一个异常时,
     * 通常会捕获第一个异常, 然后在响应中抛出第二个异常. 换句话说, 两个例外之间存在因果关系.
     * <p>
     * 相反, 在某些情况下, 可以在临近代码块中抛出两个独立的异常, 特别是在 {@code try}-with-resources
     * 语句的 {@code try} 块和编译器生成的 {@code finally} 中关闭资源的块.
     * <p>
     * 在这些情况下, 只能传播一个抛出的异常. 在 {@code try}-with-resources语句中, 当存在两个此类异常时,
     * 将传播源自 {@code try} 块的异常, 并将 {@code finally} 块中的异常添加到来自 {@code try} 块的异常
     * 所抑制的异常列表中. 作为异常展开堆栈, 它可以累积多个抑制异常.
     *
     * <p>异常可能会抑制异常, 同时也会由另一个异常引起异常. 异常是否具有原因在创建时在语义上是已知的,
     * 不像异常是否会抑制通常仅在抛出异常之后确定的其他异常.
     *
     * <p>请注意, 程序员编写的代码也能够利用在存在多个兄弟异常并且只能传播一个异常的情况下调用此方法.
     *
     * @param exception 要添加到已抑制的异常列表中的异常
     * @throws IllegalArgumentException 如果{@code exception} 是这个 Throwable 本身;
     *                                  一个 Throwable 无法抑制自己.
